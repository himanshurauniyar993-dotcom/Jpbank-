...
import { ArrowLeft, CheckCircle2, ShieldCheck, Banknote, User, ArrowRight, ShieldAlert, Lock, Delete, ChevronDown, Eye, EyeOff } from 'lucide-react';
...
export default function SendYen() {
  ...
  const handleRecipientChange = (val: string) => {
    ...
    if (!val.startsWith('@')) {
      setRecipient('@' + val.replace(/^@+/, ''));
    } else {
      // Prevent multiple @ at the start and keep the rest as is
      setRecipient('@' + val.slice(1).replace(/^@+/, ''));
    }
  };
  ...
  const handleLookup = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/user/lookup/${encodeURIComponent(recipient)}`, {
  ...
  const handleTransferSubmit = async () => {
    ...
    try {
      ...
      setSuccess(true);
      refreshUser();
      // Keep success screen visible for longer
  ...
  useEffect(() => {
    if (transactionPin.length === 4 && step === 4) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col pb-24">
      <div className="bg-[#B22222] text-white p-4 flex items-center shadow-md relative z-10">
        <button onClick={handleBack} className="p-2 hover:bg-white/10 rounded-full transition-colors mr-4">
          <ArrowLeft className="w-6 h-6" />
        </button>
        <h1 className="text-xl font-bold tracking-tight">Send Yen</h1>
      </div>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-sm relative z-10 overflow-hidden border border-gray-100">
            ...
            <form onSubmit={handleLookup} className="space-y-6">
              <div className="flex flex-col items-center mb-6">
                <div className="w-16 h-16 bg-red-50 rounded-full flex items-center justify-center mb-4 shadow-inner">
                  <User className="w-8 h-8 text-[#B22222]" />
                </div>
                <h2 className="text-xl font-bold text-gray-800 text-center">Recipient Details</h2>
                <p className="text-gray-500 text-center text-xs mt-1">Enter the recipient's @username or phone number</p>
              </div>

              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Recipient</label>
                <div className="relative">
                  <User className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
                  <input
                    type="text"
                    required
                    className="w-full pl-12 pr-4 py-4 rounded-2xl border border-gray-100 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-[#B22222] focus:border-transparent transition-all outline-none font-bold text-gray-900"
                    value={recipient}
                    onChange={(e) => handleRecipientChange(e.target.value)}
                    onFocus={() => { if (!recipient) setRecipient('@'); }}
              ...
              <button
                ...
                className="w-full bg-[#B22222] hover:bg-[#8B0000] disabled:bg-gray-300 text-white font-bold py-4 rounded-2xl shadow-lg shadow-red-100 transition-all active:scale-[0.98] flex justify-center items-center space-x-2"
              >
                {loading ? (
          ...
          {step === 2 && recipientDetails && (
            ...
            >
              ...
              <div className="bg-[#B22222] text-white p-4 flex items-center shadow-md flex-shrink-0">
                <button onClick={() => setStep(1)} className="p-2 hover:bg-white/10 rounded-full transition-colors mr-4">
                  <ArrowLeft className="w-6 h-6" />
                </button>
              ...
              {/* Main Amount Area - Improved Spacing */}
              ...
              <div className="bg-white border border-gray-100 rounded-3xl p-5 shadow-sm space-y-5 overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-50">
                  <div>
                    ...
                    <p className="text-xs font-bold text-gray-800">{purpose}</p>
                  </div>
                  <div>
          ...
          {step === 4 && recipientDetails && (
            ...
            >
              ...
              <div className="bg-[#B22222] text-white p-4 flex items-center shadow-md flex-shrink-0">
                <button onClick={() => setStep(3)} className="p-2 hover:bg-white/10 rounded-full transition-colors mr-4">
                  <ArrowLeft className="w-6 h-6" />
                </button>
              ...
              <div className="flex-1 flex flex-col items-center justify-center bg-[#f8f9fa] p-4 overflow-hidden min-h-[140px]">
                ...
                {/* Processing indicator removed as requested */}
              </div>
